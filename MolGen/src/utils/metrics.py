...
def calc_novelty(train_set: Union[str, List[str]], generated_molecules: List[str]) -> float:
    ...
    Args:
        train_set_path:
            Path to the location of the data used to train the model with or a list of the loaded molecdules
    ...
    if isinstance(train_set, str):
        ...
        if os.path.isdir(train_set):
            data_paths = os.listdir(train_set)
            for path in data_paths:
                full_path = os.path.join(train_set, path)
                paths.append(full_path)
        else:
            paths.append(train_set)
