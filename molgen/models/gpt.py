...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig) -> None:
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                ln_f=nn.LayerNorm(normalized_shape=config.n_embd, bias=config.bias),
            )
        )
    ...
    def forward(self, input_ids: torch.Tensor, labels: torch.Tensor | None = None):
        ...
        b, t = input_ids.size()
        assert t <= self.config.max_seq_len, f"Cannot forward sequence length {t}, block size {self.config.max_seq_len}"
