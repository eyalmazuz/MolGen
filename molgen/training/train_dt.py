...
class Trainer:
    ...
    def train(self, optimizer):
        ...
        for epoch in range(epoch_n, epochs):
            ...
            # good_model = (epoch > 2 and (self.test_dataset is None and (epoch % 1 == 0))) or test_loss < best_loss   #always true if no test set, otherwise only true if test loss improves
            ...
            if self.save_path is not None and good_model:
                best_loss = test_loss
                self.save_checkpoint(epoch) # ALWAYS saves with name "epoch_{epoch}.pth"
